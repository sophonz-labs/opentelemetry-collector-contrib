...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector-contrib/cmd/sophonzschemamigrator/migrationmanager"
...
func main() {
	...
	userName := os.Getenv("CLICKHOUSE_ADMIN_USERNAME")
	if userName == "" {
	...
	var dsn string
	if host == "" {
		logger.Fatal("Failed to get host from env:CLICKHOUSE_HOST")
	}
	if port == "" {
		logger.Fatal("Failed to get port from env:CLICKHOUSE_PORT")
	}
	if userName == "" || password == "" {
		dsn = fmt.Sprintf("clickhouse://%s:%s", host, port)
	} else {
		//dsn = fmt.Sprintf("tcp://%s:%s@%s:%s", userName, password, host, port)
		dsn = fmt.Sprintf("clickhouse://%s:%s?username=%s&password=%s", host, port, userName, password)
	}

	// set cluster env so that golang-migrate can use it
	// the value of this env would replace all occurences of {{.SOPHONZ_CLUSTER}} in the migration files
	// TODO: remove this log after dirtry migration issue is fixed
	logger.Info("Setting env var SOPHONZ_CLUSTER", zap.String("cluster-name", clusterName))
	err = os.Setenv("SOPHONZ_CLUSTER", clusterName)
	if err != nil {
		logger.Fatal("Failed to set env var SOPHONZ_CLUSTER", zap.Error(err))
	}
	// TODO: remove this section after dirtry migration issue is fixed
	clusterNameFromEnv := ""
	for _, kvp := range os.Environ() {
		kvParts := strings.SplitN(kvp, "=", 2)
		if kvParts[0] == "SOPHONZ_CLUSTER" {
			clusterNameFromEnv = kvParts[1]
			break
		}
	}
	if clusterName == "" {
		logger.Fatal("Failed to set env var SOPHONZ_CLUSTER")
	}
	logger.Info("Successfully set env var SOPHONZ_CLUSTER ", zap.String("cluster-name", clusterNameFromEnv))

	// set SOPHONZ_REPLICATED env var so that golang-migrate can use it
	// the value of this env would replace all occurences of {{.SOPHONZ_REPLICATED}} in the migration files
	sophonzReplicated := ""
	logger.Info("Setting env var SOPHONZ_REPLICATED", zap.Bool("replication", replicationEnabled))
	if replicationEnabled {
		sophonzReplicated = "Replicated"
	}
	err = os.Setenv("SOPHONZ_REPLICATED", sophonzReplicated)
	if err != nil {
		logger.Fatal("Failed to set env var SOPHONZ_REPLICATED", zap.Error(err))
	}
