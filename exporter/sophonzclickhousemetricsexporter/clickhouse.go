...
// Package sophonzclickhousemetricsexporter provides writer for ClickHouse storage.
...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector-contrib/exporter/sophonzclickhousemetricsexporter/base"
	chproto "github.com/open-telemetry/opentelemetry-collector-contrib/exporter/sophonzclickhousemetricsexporter/internal/chproto"
	"github.com/open-telemetry/opentelemetry-collector-contrib/exporter/sophonzclickhousemetricsexporter/utils/timeseries"
...
func (ch *clickHouse) Write(ctx context.Context, data *prompb.WriteRequest, metricNameToMeta map[string]base.MetricMeta) error {
	...
	for i, ts := range data.Timeseries {
		...
		for _, label := range ts.Labels {
			...
			if label.Name == string(semconv.DeploymentEnvironmentKey) || label.Name == sanitize(string(semconv.DeploymentEnvironmentKey)) {
				env = label.Value
			}
		}
	...
	if ch.writeTSToV4 {
			...
			for i, ts := range data.Timeseries {
				...
				for _, s := range ts.Samples {
					...
					for _, val := range timeSeries[fingerprint] {
						if val.Name == nameLabel && (strings.HasPrefix(val.Value, "sophonz_") || strings.HasPrefix(val.Value, "chi_") || strings.HasPrefix(val.Value, "otelcol_")) {
							collectUsage = false
							break
						}
						if val.Name == "tenant" {
		...
		for k, v := range metrics {
			stats.RecordWithTags(ctx, []tag.Mutator{tag.Upsert(usage.TagTenantKey, k), tag.Upsert(usage.TagExporterIdKey, ch.exporterID.String())}, ExporterSOPHONZSentMetricPoints.M(int64(v.Count)), ExporterSOPHONZSentMetricPointsBytes.M(int64(v.Size)))
		}
	}
