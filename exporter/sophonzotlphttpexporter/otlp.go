...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector-contrib/exporter/sophonzotlphttpexporter/internal/httphelper"
)
...
func (e *baseExporter) pushTraces(ctx context.Context, td ptrace.Traces) error {
	// SOPHONZ: skip entirely when the exporter is disabled.
	if !e.config.Enabled {
		return nil
	}

	// SOPHONZ: only export spans whose status code is Error.
	tdErrorOnly := ptrace.NewTraces()

	rss := td.ResourceSpans()
	for i := 0; i < rss.Len(); i++ {
		rs := rss.At(i)

		newRS := tdErrorOnly.ResourceSpans().AppendEmpty()
		...
		for j := 0; j < ilss.Len(); j++ {
			...
			for k := 0; k < spans.Len(); k++ {
				...
				if span.Status().Code() == ptrace.StatusCodeError {
					span.CopyTo(newILS.Spans().AppendEmpty())
				}
			}

			if newILS.Spans().Len() == 0 {
				newRS.ScopeSpans().RemoveIf(func(p ptrace.ScopeSpans) bool {
					return p == newILS
				})
			}
		}

		if newRS.ScopeSpans().Len() == 0 {
			tdErrorOnly.ResourceSpans().RemoveIf(func(p ptrace.ResourceSpans) bool {
				return p == newRS
			})
		}
	}

	if tdErrorOnly.ResourceSpans().Len() == 0 {
		return nil
	}

	tr := ptraceotlp.NewExportRequestFromTraces(tdErrorOnly)
...
func (e *baseExporter) pushLogs(ctx context.Context, ld plog.Logs) error {
	...
	// SOPHONZ: only export log records carrying an exception stacktrace attribute.
	filteredLd := plog.NewLogs()
	stacktraceKeyString := string(semconv.ExceptionStacktraceKey)

	rlss := ld.ResourceLogs()
	for i := 0; i < rlss.Len(); i++ {
		srcResourceLogs := rlss.At(i)

		destScopeLogsSlice := plog.NewScopeLogsSlice()

		srcScopeLogsSlice := srcResourceLogs.ScopeLogs()
		for j := 0; j < srcScopeLogsSlice.Len(); j++ {
			srcScopeLogs := srcScopeLogsSlice.At(j)

			destLogRecordSlice := plog.NewLogRecordSlice()

			srcLogRecordSlice := srcScopeLogs.LogRecords()
			for k := 0; k < srcLogRecordSlice.Len(); k++ {
				srcLogRecord := srcLogRecordSlice.At(k)
				if _, found := srcLogRecord.Attributes().Get(stacktraceKeyString); found {
			...
			if destLogRecordSlice.Len() > 0 {
				...
				srcScopeLogs.Scope().CopyTo(destScopeLogs.Scope())
				destLogRecordSlice.MoveAndAppendTo(destScopeLogs.LogRecords())
			}
		}

		if destScopeLogsSlice.Len() > 0 {
			destResourceLogs := filteredLd.ResourceLogs().AppendEmpty()
			srcResourceLogs.Resource().CopyTo(destResourceLogs.Resource())
			destScopeLogsSlice.MoveAndAppendTo(destResourceLogs.ScopeLogs())
		}
	}

	if filteredLd.ResourceLogs().Len() == 0 {
		return nil
	}

	tr := plogotlp.NewExportRequestFromLogs(filteredLd)
...
func (e *baseExporter) export(ctx context.Context, requestURL string, request []byte, partialSuccessHandler partialSuccessHandler) error {
	...
	// SOPHONZ: use the vendored httphelper status mapping for HTTP->gRPC code propagation.
